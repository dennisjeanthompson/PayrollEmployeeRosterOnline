...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/shifts/bulk-delete", requireAuth, requireRole(["manager"]), asyncHandler(async (req, res) => {
    try {
      const branchId = req.user!.branchId;
      const { startDate, endDate, employeeId, target } = req.body;
      ...
      const allLogs = await storage.getAdjustmentLogsByBranch(branchId, start, end);
      let filteredLogs = employeeId && employeeId !== 'all'
        ? allLogs.filter((l: any) => String(l.employeeId) === String(employeeId))
        : allLogs;
  ...
  // ─── BULK CREATE ──────────────────────────────────────────────────
  app.post("/api/shifts/bulk", requireAuth, requireRole(["manager"]), asyncHandler(async (req, res) => {
    try {
      ...
      while (currentDate <= end) {
        ...
        if (bulkDays.includes(dayOfWeek)) {
          // Construct start and end times for this specific day
  ...
  // ─── Admin Dashboard Stats (Company-Wide Aggregation) ───
  app.get("/api/dashboard/admin", requireAuth, requireRole(["admin"]), asyncHandler(async (req, res) => {
    try {
      ...
      await Promise.all(allBranches.map(async (branch: any) => {
        ...
        const activeUsers = branchUsers.filter((u: any) => u.isActive && (u.role === 'employee' || u.role === 'manager'));
        ...
        // 2. Pending Approvals
        const branchApprovals = await storage.getPendingApprovals(branch.id);
  ...
  app.get("/api/payroll/periods", requireAuth, requireRole(["manager"]), asyncHandler(async (req, res) => {
    try {
      ...
      const targetBranchId = (isUserAdmin && req.query.branchId && req.query.branchId !== 'all')
      ...
      if (isUserAdmin && (!req.query.branchId || req.query.branchId === 'all')) {
        ...
        await Promise.all(allBranches.map(async (b) => {
          const bp = await storage.getPayrollPeriodsByBranch(b.id);
          // Attach branchName so UI knows
          allPeriods.push(...bp.map((p: any) => ({ ...p, branchName: b.name })));
        }));
        allPeriods.sort((a, b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime());
        res.json({ periods: allPeriods });
      } else {
        const periods = await storage.getPayrollPeriodsByBranch(targetBranchId);
        res.json({ periods });
      }
    } catch (error: any) {
...
// --- Pre-fetch static branch & company data to prevent N+1 Queries ---
      ...
      // Record the computed totals but keep the period OPEN. Processing only generates
      // entries as 'pending' — a manager approves each entry and then marks it paid.
      // (Previously this auto-closed the period and marked every entry paid instantly.)
  ...
  // Live "accrued so far" preview for a period: hours worked and estimated GROSS
  // earnings for the elapsed part of the period (start → today, capped at the end
  // date). Informational only — it does NOT create entries or change payroll.
  app.get("/api/payroll/periods/:id/accrued", requireAuth, requireRole(["manager"]), asyncHandler(async (req, res) => {
    try {
      const { id } = req.params;
      const period = await storage.getPayrollPeriod(id);
      if (!period) {
      ...
      // Period hasn't started yet — nothing accrued.
      if (asOf < start) {
        return res.json({ accruedHours: 0, accruedPay: 0, asOf: start.toISOString(), employeeCount: 0, started: false, ended: false });
      }

      const employees = await storage.getUsersByBranch(period.branchId);
      const holidays = await storage.getHolidays(start, asOf);
      ...
      for (const emp of employees) {
        ...
        const shifts = await storage.getShiftsByUser(emp.id, start, asOf);
        if (shifts.length === 0) continue;
        employeesWithShifts++;
        const calc = calculatePeriodPay(shifts, rate, holidays, -1, false, start, asOf);
  ...
  app.get("/api/payroll/entries/branch", requireAuth, requireRole(["manager"]), asyncHandler(async (req, res) => {
    try {
      ...
      // Get all active employees in the target branch(es)
      let allEmployees: any[] = [];
      if (isUserAdmin && (!req.query.branchId || req.query.branchId === 'all')) {
        const allBranches = await storage.getAllBranches();
        await Promise.all(allBranches.map(async (b) => {
          const users = await storage.getUsersByBranch(b.id);
          allEmployees.push(...users.map((u: any) => ({ ...u, branchName: b.name })));
        }));
      } else {
        allEmployees = await storage.getUsersByBranch(targetBranchId);
      }
      
      const employees = allEmployees.filter((emp: any) => emp.isActive);
