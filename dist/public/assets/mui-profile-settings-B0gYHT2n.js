...
function MuiProfileSettings() {
  ...
  const updateProfileMutation = useMutation({
    ...
    onSuccess: (data) => {
      __vitePreload(async () => { const {startTransition} = await import('./vendor-D1hggvo0.js').then(n => n.R);return { startTransition }},true              ?__vite__mapDeps([0,1]):void 0).then(({ startTransition }) => {
        startTransition(() => {
          ...
          Paper,
          {
            ...
            children: /* @__PURE__ */ jsxRuntimeExports.jsx(
              Button,
              {
                ...
                onClick: async () => {
                  try {
                    const { logout } = await __vitePreload(async () => { const { logout } = await import('./main-BCIEvjoh.js').then(n => n.l);return { logout }},true              ?__vite__mapDeps([2,0,1,3]):void 0);
                    await logout();
