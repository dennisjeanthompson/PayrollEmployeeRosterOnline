...
var init_db_storage = __esm({
  "server/db-storage.ts"() {
    ...
    DatabaseStorage = class {
      ...
      // Get all admin-role users regardless of branch
      async getAdminUsers() {
      ...
      async getAvailableShiftTrades(branchId, userId) {
        ...
        return result.map((r) => r.trade).filter((t) => {
          ...
          if (userId) {
            const passed = t.passedByUserIds ?? [];
            if (passed.includes(userId)) return false;
          }
          return true;
        });
      }
      async passTradeForUser(tradeId, userId) {
        const trade = await this.getShiftTrade(tradeId);
        if (!trade) throw new Error("Trade not found");
        const current = trade.passedByUserIds ?? [];
        if (!current.includes(userId)) {
          await db.update(shiftTrades).set({ passedByUserIds: [...current, userId] }).where(eq(shiftTrades.id, tradeId));
        }
      }
    ...
    MemStorage = class {
      ...
      async getAvailableShiftTrades(branchId, userId) {
        return Array.from(this.shiftTrades.values()).filter((trade) => {
          ...
          if (userId) {
            const passed = trade.passedByUserIds ?? [];
            if (passed.includes(userId)) return false;
          }
          return true;
        });
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/shift-trades", requireAuth9, asyncHandler(async (req, res) => {
    try {
      ...
      const openTrades = await storage5.getAvailableShiftTrades(branchId, userId);
      let managerTrades = [];
      if (req.user.role === "manager" || req.user.role === "admin") {
  ...
  app2.get("/api/shift-trades/available", requireAuth9, asyncHandler(async (req, res) => {
    const branchId = req.user.branchId;
    const userId = req.user.id;
    const trades = await storage5.getAvailableShiftTrades(branchId, userId);
  ...
  app2.patch("/api/shift-trades/:id", requireAuth9, asyncHandler(async (req, res) => {
    try {
      ...
      const responder = await storage5.getUser(userId);
  ...
  app2.patch("/api/shift-trades/:id/approve", requireAuth9, requireRole3(["manager", "admin"]), asyncHandler(async (req, res) => {
    try {
      ...
      if (status === "approved") {
        const requesterMsg = didSwap ? `Your trade for the ${shiftDate} shift was approved by ${managerName}. ${toName} will cover your shift and you've been assigned their shift in return.` : `Your trade for the ${shiftDate} shift was approved by ${managerName}. ${toName} will cover your shift. No same-day swap shift was available, so this was a one-way transfer.`;
        const nReq = await storage5.createNotification({
          userId: trade.fromUserId,
          type: "shift_trade",
        ...
        realTimeManager.broadcastNotification(nReq);
        if (trade.toUserId) {
          const targetMsg = didSwap ? `You've been assigned ${fromName}'s ${shiftDate} shift (approved trade). Your same-day shift has been moved to ${fromName}.` : `You've been assigned ${fromName}'s ${shiftDate} shift from an approved trade.`;
  ...
  app2.get("/api/notifications", requireAuth9, asyncHandler(async (req, res) => {
    try {
      ...
      const branchFiltered = isAdminUser ? notifications2 : notifications2.filter((n) => !n.branchId || n.branchId === activeBranchId);
