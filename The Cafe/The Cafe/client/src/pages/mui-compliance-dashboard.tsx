...
export default function MuiComplianceDashboard() {
  ...
  complianceChecks.push({
    ...
    message: sssRates.length >= 30 
      ? `${sssRates.length} brackets configured (compliant)`
      : sssRates.length > 0 
        ? `Only ${sssRates.length} brackets. Should be 33 for full compliance.`
        : "No SSS rates configured. Add current rate brackets.",
    icon: <SecurityIcon />,
  });
  ...
  const minWageLaUnion = 470; // ₱470/day La Union
    ...
    <Box sx={{ p: { xs: 2, md: 4 } }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 4 }}>
        ...
        <Box sx={{ flex: 1 }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Philippine Payroll Compliance Status
          </Typography>
        </Box>
        <Chip
      ...
      <Alert severity="info" sx={{ mt: 4, borderRadius: 2 }}>
        <Typography variant="body2" gutterBottom>
          <strong>Current Rate References:</strong>
        </Typography>
